...
export const CommissionDetailContent: React.FC<CommissionDetailContentProps> = ({
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden bg-white/50 dark:bg-black/20 backdrop-blur-md rounded-2xl border border-white/10 shadow-xl">
            {/* HEADER */}
            <div className="p-4 border-b border-gray-200 dark:border-white/10 bg-gray-50/50 dark:bg-black/20">
                <div className="flex justify-between items-start">
                    ...
                    {onClose && (
                        <button onClick={onClose} className="p-2 bg-gray-100 dark:bg-white/5 rounded-full text-gray-500 dark:text-white/60 hover:text-gray-900 dark:hover:text-white ml-3">
                            <X size={20} />
                        </button>
                    )}
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
                ...
                {onSaveOfficeData && (
                    <div className="mt-8 border-t border-gray-200 dark:border-white/10 pt-6">
                        <div className="bg-gray-50 dark:bg-black/20 rounded-xl p-4 space-y-4 border border-gray-200 dark:border-white/5">
                            ...
                            {/* Checkbox */}
                            <div
                                className={`p-3 rounded-xl border cursor-pointer transition-all flex items-center gap-3 ${isProcessed ? 'bg-emerald-100 dark:bg-emerald-500/20 border-emerald-200 dark:border-emerald-500' : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/10'}`}
                                onClick={() => setIsProcessed(!isProcessed)}
                            >
                                <div className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 transition-colors ${isProcessed ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-gray-300 dark:border-white/30'}`}>
                                    {isProcessed && <Check size={14} />}
                                </div>
                                <div>
                                    <div className="font-bold text-gray-900 dark:text-white text-sm">Büro: Gesehen / Bearbeitet</div>
                                    <div className="text-xs text-gray-500 dark:text-white/50">Vorgang als erledigt markieren.</div>
                                </div>
                            </div>

                            {/* Notes */}
                            <div>
                                <label className="text-xs text-gray-500 dark:text-white/50 font-bold uppercase mb-2 block">
                                    Interne Notizen
                                </label>
                                <textarea
                                    className="w-full bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-3 text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-emerald-500/50 min-h-[80px]"
                                ...
                                <div className="flex gap-2 mt-2 overflow-x-auto no-scrollbar pb-1">
                                    {['Termin vereinbart', 'Kunde informiert', 'Abholung bestätigt', 'Großhändler beauftragt'].map(tag => (
                                        <button
                                            ...
                                            className="px-2 py-1 rounded-full bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-xs text-gray-600 dark:text-white/60 hover:bg-gray-50 dark:hover:bg-white/10 whitespace-nowrap"
                                        >
                                            + {tag}
                                        </button>
                                    ))}
                ...
                <div className="mt-8 border-t border-gray-200 dark:border-white/10 pt-6">
                    ...
                    <div className="bg-gray-50 dark:bg-black/20 rounded-xl p-4 max-h-60 overflow-y-auto space-y-3 custom-scrollbar">
