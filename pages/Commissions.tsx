...
const Commissions: React.FC = () => {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 animate-in slide-in-from-top-2">
                      {items.map(comm => {
                          ...
                          return (
                              ...
                              >
                                  <div className={`absolute top-0 left-0 w-1 h-full ${hasItemBackorder ? 'bg-red-500' : colorClass.replace('text-', 'bg-')}`}/>
  ...
  return (
    <div className="space-y-6 pb-24">
          ...
          <div className="grid grid-cols-1 gap-4">
              ...
              {activeTab === 'active' && (
                  <>
                      ...
                      {renderCategory("In Vorbereitung", 'preparing', commissions.filter(c => c.status === 'Preparing'), 'text-blue-400')}
      ...
      {/* GLOBAL SEARCH MODAL */}
      {showSearchModal && (
          <div className="fixed inset-0 z-[150] flex items-start sm:items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
               <div className="w-full max-w-lg bg-[#1a1d24] border border-white/10 rounded-2xl shadow-2xl overflow-hidden mt-10 sm:mt-0">
                   <div className="p-4 border-b border-white/10 flex gap-3 items-center">
                       <Search className="text-white/50" size={20}/>
                       <input 
                           autoFocus
                           className="bg-transparent border-none text-white placeholder-white/30 text-lg flex-1 focus:outline-none"
                           placeholder="Global suchen (Name, Nr, Notiz)..."
                           value={globalSearchTerm}
                           onChange={(e) => performGlobalSearch(e.target.value)}
                       />
                       <button onClick={() => setShowSearchModal(false)} className="text-white/50 hover:text-white"><X size={24}/></button>
                   </div>
                   ...
                   <div className="max-h-[60vh] overflow-y-auto p-2 space-y-2">
                       ...
                       {!isSearching && globalSearchTerm && searchResults.length === 0 && (
                           <div className="text-center py-8 text-white/30">Keine Ergebnisse gefunden.</div>
                       )}
                       {searchResults.map(comm => (
                           <div 
                               key={comm.id} 
                               onClick={() => handleSearchResultClick(comm)}
                               className="p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 cursor-pointer transition-all flex justify-between items-center group"
                           >
                               <div>
                                   <div className="font-bold text-white">{comm.name}</div>
                                   <div className="flex gap-2 text-xs text-white/50 mt-1">
                                       <span className="font-mono bg-white/10 px-1.5 rounded">{comm.order_number || '-'}</span>
      ...
      {/* HISTORY MODAL */}
      {showHistoryModal && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-0 sm:p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
               <div className="w-full h-full sm:h-auto sm:max-h-[85vh] sm:max-w-2xl bg-[#1a1d24] border border-white/10 sm:rounded-2xl flex flex-col shadow-2xl">
                  <div className="p-4 border-b border-white/10 flex justify-between items-center bg-black/20">
                      <div className="flex items-center gap-2">
                          <History size={20} className="text-blue-400"/>
                          <h2 className="text-xl font-bold text-white">Verlauf / Logbuch</h2>
                      </div>
                      <button onClick={() => setShowHistoryModal(false)} className="p-2 bg-white/5 rounded-full text-white/60 hover:text-white"><X size={20}/></button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-3">
                      {loadingHistory ? (
                          ...
                          (() => {
                              ...
                              if (filteredHistory.length === 0) return <div className="text-center text-white/30 py-10">Keine Einträge gefunden.</div>;
                              ...
                              return filteredHistory.map(log => (
                                  <div key={log.id} className="bg-white/5 rounded-xl p-3 border border-white/5 flex gap-3 items-start">
                                      <div className={`mt-1 w-8 h-8 rounded-full flex items-center justify-center shrink-0 
                                          ${log.action === 'labels_printed' ? 'bg-blue-500/20 text-blue-300' : 
                                            log.action === 'created' ? 'bg-emerald-500/20 text-emerald-300' :
                                            log.action === 'deleted' ? 'bg-rose-500/20 text-rose-300' :
                                      ...
                                      <div className="flex-1 min-w-0">
                                          <div className="flex justify-between">
                                              <span className="font-bold text-white truncate">{log.commission_name || 'Unbekannt'}</span>
                                              <span className="text-xs text-white/30 whitespace-nowrap ml-2">{new Date(log.created_at).toLocaleString()}</span>
                                          </div>
      ...
      {/* CREATE / EDIT MODAL */}
      {showCreateModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-6 bg-black/90 backdrop-blur-md animate-in fade-in">
               <div className="w-full h-full bg-[#0f172a] sm:rounded-2xl shadow-2xl flex flex-col sm:flex-row overflow-y-auto sm:overflow-hidden border border-white/10">
                  <div className="shrink-0 sm:flex-1 flex flex-col border-b sm:border-b-0 sm:border-r border-white/10 min-w-0">
                      <div className="p-6 border-b border-white/10 bg-white/5">
                          <h2 className="text-xl font-bold text-white mb-2">{isEditMode ? 'Kommission bearbeiten' : 'Neue Kommission erstellen'}</h2>
                      ...
                      <div className="h-auto sm:flex-1 sm:overflow-y-auto p-6 space-y-6">
                          <div className="space-y-4">
                              <div>
                                  <label className="text-xs text-white/50 mb-1 block">Auftrags-Nr.</label>
                                  <input className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:ring-1 focus:ring-emerald-500/20" value={newComm.order_number} onChange={e => setNewComm({...newComm, order_number: e.target.value})} autoFocus={!isEditMode}/>
                              </div>
                              <div>
                                  <label className="text-xs text-white/50 mb-1 block">Name</label>
                                  <input className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:ring-1 focus:ring-emerald-500/20" value={newComm.name} onChange={e => setNewComm({...newComm, name: e.target.value})}/>
                              </div>
                              <div>
                                  <label className="text-xs text-white/50 mb-1 block">Notizen</label>
                                  <textarea className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:ring-1 focus:ring-emerald-500/20" rows={3} value={newComm.notes} onChange={e => setNewComm({...newComm, notes: e.target.value})}/>
                              </div>
                          </div>
                          ...
                          <div className="bg-white/5 rounded-xl border border-white/5 overflow-hidden">
                              <div className="flex items-center justify-between p-2 gap-2">
                                  <button onClick={() => setExpandStockSearch(!expandStockSearch)} className="flex items-center gap-2 text-sm font-medium text-white/80 py-2 px-1 flex-shrink-0">
                                      <Package size={16}/> Material aus Hauptlager
                                  </button>
                                  
                                  {/* Inline Search Box */}
                                  <div className="flex-1 flex items-center bg-black/20 rounded-lg px-2 border border-white/5 focus-within:border-emerald-500/50 transition-colors">
                                      <Search size={14} className="text-white/40"/>
                                      <input 
                                          className="w-full bg-transparent border-none text-xs text-white placeholder-white/30 focus:ring-0 px-2 py-2"
                                          placeholder="Suchen..."
                                  ...
                                  <button onClick={() => setExpandStockSearch(!expandStockSearch)} className="p-2 text-white/50 hover:text-white">
                              ...
                              {expandStockSearch && (
                                  <div className="space-y-3 p-3 pt-0 animate-in slide-in-from-top-2">
                                      ...
                                      {/* Category Chips */}
                                      <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar border-b border-white/5 mb-2">
                                          {distinctCategories.map(cat => (
                                              <button
                                                  key={cat}
                                                  onClick={() => setSelectedCategory(selectedCategory === cat ? null : cat)}
                                                  className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${selectedCategory === cat ? 'bg-emerald-500 text-white border-emerald-500' : 'bg-white/5 border-white/10 text-white/60 hover:text-white'}`}
                                              >
                                                  {cat}
                                              </button>
                                          ))}
                                      ...
                                      <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
                                          {availableArticles
                                              ...
                                              .map(art => (
                                                  <button key={art.id} onClick={() => addTempStockItem(art)} className="w-full text-left px-3 py-2 bg-white/5 hover:bg-white/10 rounded-xl border border-white/5 flex items-center gap-3 group transition-colors">
                                                      {/* Image Thumbnail */}
                                                      <div className="w-9 h-9 shrink-0 rounded-lg bg-black/30 overflow-hidden border border-white/10">
                                                          <img 
                                                      ...
                                                      <div className="min-w-0 flex-1">
                                                          <div className="font-medium text-sm text-white truncate">{art.name}</div>
                                                          <div className="text-[10px] text-white/40 flex items-center gap-2">
                                                              <span>{art.sku}</span>
                                                              {art.stock > 0 ? <span className="text-emerald-400">Bestand: {art.stock}</span> : <span className="text-rose-400">Leer</span>}
                                                          </div>
                                                      </div>
                                                      <div className="w-6 h-6 rounded-full border border-white/10 flex items-center justify-center text-white/30 group-hover:bg-emerald-500 group-hover:text-white group-hover:border-emerald-500 transition-colors shrink-0">
                                                          <Plus size={14}/>
                                                      </div>
                                                  </button>
                                              ))
                                          }
                                          {availableArticles.length === 0 && <div className="text-center text-xs text-white/30 py-4">Kein Material im Lager.</div>}
                                      </div>
                                  </div>
                              )}
                          </div>
                          ...
                          <div className="bg-white/5 rounded-xl border border-white/5 overflow-hidden">
                              <div className="flex items-center justify-between p-2 gap-2">
                                  <button onClick={() => setExpandSupplierList(!expandSupplierList)} className="flex items-center gap-2 text-sm font-medium text-white/80 py-2 px-1 flex-shrink-0">
                                      <ShoppingCart size={16}/> Lieferant wählen
                                  </button>

                                  {/* Inline Search Box */}
                                  <div className="flex-1 flex items-center bg-black/20 rounded-lg px-2 border border-white/5 focus-within:border-purple-500/50 transition-colors">
                                      <Search size={14} className="text-white/40"/>
                                      <input 
                                          className="w-full bg-transparent border-none text-xs text-white placeholder-white/30 focus:ring-0 px-2 py-2"
                                          placeholder="Suche..."
                                  ...
                                  <div className="flex gap-1">
                                      <button onClick={() => setExpandSupplierList(!expandSupplierList)} className="p-2 text-white/50 hover:text-white">
                                          <ChevronDown size={16} className={`transition-transform ${!expandSupplierList ? '-rotate-90' : ''}`}/>
                                      </button>
                                      <button onClick={addManualItem} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg text-white/80 flex items-center justify-center ml-1" title="Freitext Position hinzufügen">
                                          <PenTool size={16}/>
                                      </button>
                                  </div>
                              </div>
                              ...
                              {expandSupplierList && (
                                  <div className="bg-black/20 p-1 animate-in slide-in-from-top-2 border-t border-white/5 max-h-40 overflow-y-auto">
                                      {suppliers
                                          .filter(s => !supplierSearchTerm || s.name.toLowerCase().includes(supplierSearchTerm.toLowerCase()))
                                          .map(s => (
                                          <button key={s.id} onClick={() => addTempExternalItem(s)} className="w-full flex items-center gap-3 p-3 hover:bg-white/5 transition-colors text-left border-b last:border-b-0 border-white/5">
                                              <Plus size={16} className="text-white/50"/> <span className="font-medium text-white/80">{s.name}</span>
                                          </button>
                                      ))}
                                  </div>
                              )}
                  ...
                  <div className="shrink-0 sm:flex-[0.8] bg-black/20 border-l border-white/10 flex flex-col min-w-0">
                      <div className="p-6 border-b border-white/10 flex justify-between items-center">
                          <h3 className="font-bold text-white">Material ({tempItems.length})</h3>
                          <button onClick={handleCloseCreateModal} className="text-white/50 hover:text-white p-2 bg-white/5 rounded-full"><X size={18}/></button>
                      </div>
                      <div className="h-auto sm:flex-1 sm:overflow-y-auto p-6 space-y-3">
                          {tempItems.length === 0 && <div className="h-full flex flex-col items-center justify-center text-white/30"><Box size={40} className="mb-3 opacity-50"/><p>Leer.</p></div>}
                          {tempItems.map((item) => (
                              <div key={item.uniqueId} className="bg-white/5 rounded-xl p-4 shadow-lg border border-white/10">
                                  <div className="flex justify-between items-start mb-3">
                                      <div className="flex-1 min-w-0 pr-3">
                                          {item.type === 'Stock' ? (
                                              <>
                                                  <div className="font-bold text-white truncate">{item.article?.name}</div>
                                              ...
                                              <>
                                                  <div className="text-[10px] text-purple-300 uppercase font-bold mb-1">Manuell / Extern</div>
                                                  <input 
                                                      className="w-full bg-transparent border-b border-white/20 text-white font-bold focus:outline-none focus:border-purple-500 pb-1" 
                                      ...
                                      <div className="flex items-center gap-2 shrink-0">
                                          <button onClick={() => updateTempItem(item.uniqueId, 'amount', Math.max(1, item.amount - 1))} className="w-7 h-7 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center text-white"><Minus size={14}/></button>
                                          <span className="w-8 text-center font-bold text-white">{item.amount}</span>
                                          <button onClick={() => updateTempItem(item.uniqueId, 'amount', item.amount + 1)} className="w-7 h-7 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center text-white"><Plus size={14}/></button>
                                          <button onClick={() => removeTempItem(item.uniqueId)} className="ml-2 text-white/30 hover:text-rose-400"><X size={18}/></button>
                                      </div>
                                  </div>
                                  ...
                                  {item.type === 'External' && (
                                      <div className="mb-2">
                                          <label className="text-[10px] text-white/40 uppercase font-bold whitespace-nowrap block mb-1">Notiz an Lieferant / Lager:</label>
                                          <input 
                                              className="w-full bg-black/30 text-white text-xs p-2 rounded-lg border border-white/10 focus:outline-none focus:border-white/30"
                                  ...
                                  {item.type === 'External' && (
                                      ...
                                      >
                                          <div className="bg-black/30 rounded-lg p-2 border border-white/10 flex-1 flex items-center gap-2">
                                              <label className="text-[10px] text-white/40 uppercase font-bold whitespace-nowrap">Vorgang:</label>
                                              <input className="w-full bg-transparent text-white text-sm focus:outline-none font-mono" placeholder="Optional..." value={item.externalReference || ''} onChange={(e) => updateTempItem(item.uniqueId, 'externalReference', e.target.value)}/>
                                          </div>
                                          
                                          {/* PASTE BUTTON */}
                                          <button 
                                              onClick={() => handlePasteAttachment(item.uniqueId)}
                                              className="h-10 w-10 rounded-lg flex items-center justify-center border bg-white/10 text-white/40 border-white/10 hover:bg-white/20 hover:text-white transition-all shrink-0"
                                          ...
                                          <div>
                                              ...
                                              <button 
                                                  ...
                                                  className={`h-10 w-10 rounded-lg flex items-center justify-center border transition-all shrink-0 ${item.attachmentData ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50' : 'bg-white/10 text-white/40 border-white/10 hover:bg-white/20 hover:text-white'}`}
      ...
      {showLabelOptionsModal && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
              <GlassCard className="w-full max-w-md text-center">
                  <Tag size={48} className="mx-auto text-blue-400 mb-4"/>
                  <h2 className="text-xl font-bold text-white mb-2">Kommission erstellt!</h2>
                  <p className="text-white/60 mb-6">Möchtest du direkt ein Etikett für das Regal/die Box drucken?</p>
      ...
      {showLabelUpdateModal && activeCommission && (
          <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
              <GlassCard className="w-full max-w-md text-center">
                  ...
                  <h2 className="text-xl font-bold text-white mb-2">Etikett aktualisieren?</h2>
                  <p className="text-white/60 mb-6">
                      Du hast Änderungen an Rückständen oder Notizen vorgenommen. Das Etikett ist nicht mehr aktuell.
                  </p>
                  
                  <div className="space-y-3">
                      <Button onClick={() => handleSinglePrint(activeCommission.id)} className="w-full py-3 bg-emerald-600 hover:bg-emerald-500" icon={<Printer size={18}/>}>Neues Etikett drucken</Button>
                      <Button onClick={() => handleAddToQueue(activeCommission.id, activeCommission.name)} className="w-full py-3 bg-blue-600 hover:bg-blue-500" icon={<Layers size={18}/>}>Zur Druckwarteschlange</Button>
      ...
      {editingItemNote && (
          <div className="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
              <GlassCard className="w-full max-w-sm">
                  <div className="flex justify-between items-center mb-4">
                      <h3 className="font-bold text-white">Notiz zur Position</h3>
                      <button onClick={() => setEditingItemNote(null)} className="text-white/50 hover:text-white"><X size={20}/></button>
                  </div>
                  <textarea 
                      className="w-full h-32 bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  ...
                  <div className="flex justify-end gap-3 mt-4">
                      ...
                      <Button onClick={() => {
                          ...
                          saveItemNote(editingItemNote.itemId, val);
                      }}>Speichern</Button>
                  </div>
              </GlassCard>
          </div>
      ...
      {viewingAttachment && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md animate-in fade-in">
              <div className="w-full max-w-4xl h-[90vh] bg-gray-900 border border-white/10 rounded-2xl flex flex-col relative overflow-hidden">
      ...
      {/* DETAIL MODAL */}
      {showPrepareModal && activeCommission && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-4 bg-black/90 backdrop-blur-md animate-in fade-in zoom-in-95">
               <div className="w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-3xl bg-[#1a1d24] border border-white/10 sm:rounded-2xl flex flex-col overflow-hidden shadow-2xl">
                  <div className="p-4 border-b border-white/10 bg-black/20">
                      <div className="flex justify-between items-start">
                          <div className="flex-1">
                              <h2 className="text-xl font-bold text-white mb-1">{activeCommission.name}</h2>
                              ...
                              {activeCommission.notes && (
                                  <div className="text-sm text-white/70 mt-3 bg-white/5 p-3 rounded-lg whitespace-pre-wrap border border-white/5">
                                      {activeCommission.notes}
                                  </div>
                              )}
                          </div>
                          <button onClick={handleClosePrepare} className="p-2 bg-white/5 rounded-full text-white/60 hover:text-white ml-3"><X size={20}/></button>
                      </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-6">
                      ...
                      {!activeCommission.status.startsWith('Return') && activeCommission.status !== 'Withdrawn' && activeCommission.deleted_at === null && (
                          <div className="flex flex-wrap gap-2 pb-2 border-b border-white/5 mb-4">
                              {activeCommission.status !== 'Ready' && (
                                  <div className="relative group">
                                      <Button 
                                        onClick={handleSetReadyTrigger} 
                                        className={`whitespace-nowrap ${(!allItemsPicked || hasBackorders) ? 'bg-gray-600 opacity-50 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-500'}`} 
                      ...
                      <div className="space-y-2">
                          {commItems.length === 0 && <div className="text-center text-white/30 py-4">Keine Positionen.</div>}
                          {commItems.map(item => (
                              <div key={item.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${item.is_backorder ? 'bg-red-500/10 border-red-500/50' : item.is_picked ? 'bg-emerald-500/10 border-emerald-500/30 opacity-70' : 'bg-white/5 border-white/5'}`}>
                                  ...
                                  <div className={`flex-1 ${item.is_backorder ? 'cursor-default' : 'cursor-pointer'}`} onClick={() => !item.is_backorder && toggleActiveItemPicked(item.id, item.is_picked)}>
                                      ...
                                      <div className="text-xs text-white/50 flex flex-wrap items-center gap-2">
                                          ...
                                          {item.notes && <span className="flex items-center gap-1 text-amber-300"><MessageSquare size={10}/> {item.notes}</span>}
                                      </div>
                                  </div>
                                  ...
                                  <div className="flex items-center gap-2">
                                      ...
                                      <button 
                                          ...
                                          className={`p-1.5 rounded transition-colors ${item.notes ? 'text-amber-400 bg-amber-500/20' : 'text-white/30 hover:text-white'}`}
                                      ...
                                      {/* Backorder Toggle */}
                                      {item.type === 'External' && (
                                          <div className="flex items-center gap-2 bg-black/20 p-1 rounded-lg border border-white/5" title="Rückstand umschalten">
                                              <span className={`text-[9px] uppercase font-bold px-1 ${item.is_backorder ? 'text-red-400' : 'text-white/30'}`}>Rückstd.</span>
                                              <button 
                                                  onClick={(e) => { e.stopPropagation(); toggleBackorder(item.id, item.is_backorder || false); }}
                                                  className={`w-8 h-4 rounded-full relative transition-colors ${item.is_backorder ? 'bg-red-500' : 'bg-white/20'}`}
                                              >
                                      ...
                                      <div className="font-bold text-white text-lg px-2">{item.amount}x</div>
                                  </div>
                              </div>
                          ))}
                      </div>
      ...
      {showConfirmReadyModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
              <GlassCard className="w-full max-w-sm">
                  <div className="flex items-center gap-3 mb-3 text-emerald-400"><AlertTriangle size={24} /><h3 className="text-lg font-bold text-white">Bereitstellen bestätigen?</h3></div>
                  <p className="text-sm text-white/70 mb-6">Bestand für {commItems.filter(i => i.type === 'Stock').length} Artikel wird abgebucht.</p>
      ...
      {showConfirmWithdrawModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
              <GlassCard className="w-full max-w-sm">
                  <div className="flex items-center gap-3 mb-3 text-purple-400"><Truck size={24} /><h3 className="text-lg font-bold text-white">Entnahme bestätigen?</h3></div>
                  <p className="text-sm text-white/70 mb-6">Kommission wird als "Entnommen" markiert und archiviert.</p>
                  <div className="flex gap-3 justify-end">
      ...
      {deleteTarget && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
              <GlassCard className="w-full max-w-sm text-center">
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${deleteTarget.mode === 'trash' ? 'bg-rose-500/10 text-rose-400' : 'bg-red-600/20 text-red-500'}`}>
                      <Trash2 size={32} />
                  </div>
