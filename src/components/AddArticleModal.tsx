...
export const AddArticleModal: React.FC<AddArticleModalProps> = ({
    ...
    return (
        <>
            <div className="fixed inset-0 z-[170] flex items-center justify-center sm:p-4 bg-black/80 backdrop-blur-md overflow-hidden">
                <div className="w-full h-full sm:h-auto sm:max-h-[90vh] max-w-2xl bg-[#1a1d24] border-0 sm:border border-white/10 sm:rounded-2xl shadow-2xl flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 sm:space-y-8">
                        ...
                        <div className="bg-black/20 rounded-xl p-4 border border-white/5">
                            <div className="flex justify-between items-center mb-3"><label className="text-xs text-white/50 font-bold uppercase tracking-wider">Lieferanten</label><span className="text-xs text-white/30">{tempSuppliers.length}</span></div>
                            <div className="space-y-3"><div className="flex gap-2"><GlassSelect className="flex-1 py-2 text-sm" value={tempSupplierSelect} onChange={(e) => setTempSupplierSelect(e.target.value)}><option value="" className="bg-gray-900 text-white">Lieferant auswählen...</option>{suppliers.map(s => <option key={s.id} value={s.id} className="bg-gray-900 text-white">{s.name}</option>)}</GlassSelect><Button type="button" onClick={addTempSupplier} disabled={!tempSupplierSelect} className="px-3 py-2 h-auto"><Plus size={16} /></Button></div><div className="grid grid-cols-2 gap-2"><input className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:outline-none placeholder:text-white/30" placeholder="Art-Nr. beim Lieferant" value={tempSupplierSkuInput} onChange={e => setTempSupplierSkuInput(e.target.value)} /><input className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:outline-none placeholder:text-white/30" placeholder="Produkt-Link (URL)" value={tempSupplierUrlInput} onChange={e => setTempSupplierUrlInput(e.target.value)} /></div></div>
                            <div className="space-y-2 mt-3">{tempSuppliers.length === 0 && <div className="text-center text-xs text-white/30 py-2">Keine weiteren Lieferanten</div>}{tempSuppliers.map((s, idx) => (<div key={idx} className={`flex items-center justify-between p-3 rounded-xl border ${s.isPreferred ? 'bg-blue-500/10 border-blue-500/30' : 'bg-white/5 border-white/10'}`}><div className="flex-1 min-w-0 mr-4"><div className="flex items-center gap-2"><span className="font-bold text-sm text-white">{s.supplierName}</span>{s.isPreferred && <span className="text-[9px] bg-blue-500 text-white px-1.5 rounded-sm">Primär</span>}</div>{editingSupplierIndex === idx ? (<div className="grid grid-cols-2 gap-2 mt-2"><input className="bg-black/30 border border-white/20 rounded px-2 py-1 text-xs text-white" value={editSupplierData.supplierSku} onChange={e => setEditSupplierData({ ...editSupplierData, supplierSku: e.target.value })} placeholder="Art-Nr." /><input className="bg-black/30 border border-white/20 rounded px-2 py-1 text-xs text-white" value={editSupplierData.url} onChange={e => setEditSupplierData({ ...editSupplierData, url: e.target.value })} placeholder="URL" /><div className="col-span-2 flex justify-end gap-2"><button onClick={saveEditingSupplier} className="text-xs text-emerald-400 hover:underline">Speichern</button><button onClick={() => setEditingSupplierIndex(null)} className="text-xs text-white/50 hover:underline">Abbruch</button></div></div>) : (<div className="text-xs text-white/50 flex flex-col sm:flex-row sm:gap-3 mt-0.5"><span className="truncate">Art-Nr: {s.supplierSku || '-'}</span>{s.url && <a href={s.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-blue-400 hover:underline truncate max-w-[150px]"><LinkIcon size={10} /> Link</a>}</div>)}</div><div className="flex items-center gap-1">{editingSupplierIndex !== idx && (<><button type="button" onClick={() => startEditingSupplier(idx)} className="p-2 hover:bg-white/10 rounded-lg text-white/40 hover:text-white"><Edit size={14} /></button><button type="button" onClick={() => togglePreferredSupplier(idx)} className={`p-2 hover:bg-white/10 rounded-lg ${s.isPreferred ? 'text-blue-400' : 'text-white/20 hover:text-white'}`} title="Als Hauptlieferant setzen"><Star size={14} fill={s.isPreferred ? "currentColor" : "none"} /></button><button type="button" onClick={() => removeTempSupplier(idx)} className="p-2 hover:bg-white/10 rounded-lg text-white/40 hover:text-rose-400"><Trash2 size={14} /></button></>)}</div></div>))}</div>
                        </div>
                    </div>
                    <div className="p-4 sm:p-6 border-t border-white/10 flex justify-end gap-3 bg-gray-900/95 sm:bg-black/40 rounded-none sm:rounded-b-2xl sticky bottom-0 z-10 backdrop-blur-xl shrink-0">
                        <Button variant="secondary" onClick={onClose}>Abbrechen</Button>
                        <Button variant="secondary" onClick={() => handleSaveArticle()} disabled={isSubmitting} className="border-emerald-500/30 text-emerald-300 hover:bg-emerald-500/20">Speichern & Weiter</Button>
            ...
            {/* AI SCAN MODAL */}
            {isAiModalOpen && (
                <div className="fixed inset-0 z-[180] flex items-center justify-center p-4 bg-black/90 backdrop-blur-lg animate-in fade-in zoom-in-95">
            ...
            {isImageReuseModalOpen && (
                <div className="fixed inset-0 z-[190] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in zoom-in-95">
            ...
            {duplicateConfirmation && duplicateConfirmation.isOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in zoom-in-95">
