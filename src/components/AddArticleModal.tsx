...
export const AddArticleModal: React.FC<AddArticleModalProps> = ({
    ...
    return (
        <>
            <div className="fixed inset-0 z-[170] flex items-center justify-center sm:p-4 bg-black/30 backdrop-blur-sm overflow-hidden">
                <div className="w-full h-full sm:h-auto sm:max-h-[90vh] max-w-2xl bg-[#1a1d24] border-0 sm:border border-border sm:rounded-2xl shadow-2xl flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 sm:space-y-8">
                        ...
                        <div className="bg-black/20 rounded-xl p-4 border border-white/5">
                            ...
                            <div className="space-y-3"><div className="flex gap-2"><GlassSelect className="flex-1 py-2 text-sm" value={tempSupplierSelect} onChange={(e) => setTempSupplierSelect(e.target.value)}><option value="" className="bg-background text-white">Lieferant auswählen...</option>{suppliers.map(s => <option key={s.id} value={s.id} className="bg-background text-white">{s.name}</option>)}</GlassSelect><Button type="button" onClick={addTempSupplier} disabled={!tempSupplierSelect} className="px-3 py-2 h-auto"><Plus size={16} /></Button></div><div className="grid grid-cols-2 gap-2"><input className="bg-muted border border-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none placeholder:text-muted-foreground" placeholder="Art-Nr. beim Lieferant" value={tempSupplierSkuInput} onChange={e => setTempSupplierSkuInput(e.target.value)} /><input className="bg-muted border border-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none placeholder:text-muted-foreground" placeholder="Produkt-Link (URL)" value={tempSupplierUrlInput} onChange={e => setTempSupplierUrlInput(e.target.value)} /></div></div>
                            <div className="space-y-2 mt-3">{tempSuppliers.length === 0 && <div className="text-center text-xs text-muted-foreground py-2">Keine weiteren Lieferanten</div>}{tempSuppliers.map((s, idx) => (<div key={idx} className={`flex items-center justify-between p-3 rounded-xl border ${s.isPreferred ? 'bg-blue-500/10 border-blue-500/30' : 'bg-muted border-border'}`}><div className="flex-1 min-w-0 mr-4"><div className="flex items-center gap-2"><span className="font-bold text-sm text-white">{s.supplierName}</span>{s.isPreferred && <span className="text-[9px] bg-blue-500 text-white px-1.5 rounded-sm">Primär</span>}</div>{editingSupplierIndex === idx ? (<div className="grid grid-cols-2 gap-2 mt-2"><input className="bg-black/30 border border-border rounded px-2 py-1 text-xs text-white" value={editSupplierData.supplierSku} onChange={e => setEditSupplierData({ ...editSupplierData, supplierSku: e.target.value })} placeholder="Art-Nr." /><input className="bg-black/30 border border-border rounded px-2 py-1 text-xs text-white" value={editSupplierData.url} onChange={e => setEditSupplierData({ ...editSupplierData, url: e.target.value })} placeholder="URL" /><div className="col-span-2 flex justify-end gap-2"><button onClick={saveEditingSupplier} className="text-xs text-emerald-400 hover:underline">Speichern</button><button onClick={() => setEditingSupplierIndex(null)} className="text-xs text-muted-foreground hover:underline">Abbruch</button></div></div>) : (<div className="text-xs text-muted-foreground flex flex-col sm:flex-row sm:gap-3 mt-0.5"><span className="truncate">Art-Nr: {s.supplierSku || '-'}</span>{s.url && <a href={s.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-blue-400 hover:underline truncate max-w-[150px]"><LinkIcon size={10} /> Link</a>}</div>)}</div><div className="flex items-center gap-1">{editingSupplierIndex !== idx && (<><button type="button" onClick={() => startEditingSupplier(idx)} className="p-2 hover:bg-muted rounded-lg text-muted-foreground hover:text-white"><Edit size={14} /></button><button type="button" onClick={() => togglePreferredSupplier(idx)} className={`p-2 hover:bg-muted rounded-lg ${s.isPreferred ? 'text-blue-400' : 'text-muted-foreground hover:text-white'}`} title="Als Hauptlieferant setzen"><Star size={14} fill={s.isPreferred ? "currentColor" : "none"} /></button><button type="button" onClick={() => removeTempSupplier(idx)} className="p-2 hover:bg-muted rounded-lg text-muted-foreground hover:text-rose-400"><Trash2 size={14} /></button></>)}</div></div>))}</div>
                        </div>
                    </div>
                    <div className="p-4 sm:p-6 border-t border-border flex justify-end gap-3 bg-background/95 sm:bg-black/40 rounded-none sm:rounded-b-2xl sticky bottom-0 z-10 backdrop-blur-sm shrink-0">
                        <Button variant="secondary" onClick={onClose}>Abbrechen</Button>
                        <Button variant="secondary" onClick={() => handleSaveArticle()} disabled={isSubmitting} className="border-emerald-500/30 text-emerald-300 hover:bg-primary/20">Speichern & Weiter</Button>
            ...
            {/* AI SCAN MODAL */}
            {isAiModalOpen && (
                <div className="fixed inset-0 z-[180] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-in fade-in zoom-in-95">
                    <GlassCard className="w-full max-w-lg flex flex-col p-0 overflow-hidden">
                        <div className="p-5 border-b border-border bg-gradient-to-r from-purple-500/10 to-blue-500/10 flex justify-between items-center">
                            ...
                            <button onClick={() => setIsAiModalOpen(false)} className="text-muted-foreground hover:text-white"><X size={20} /></button>
                        </div>
                        <div className="p-6 flex flex-col gap-4">
                            <div className="flex bg-muted p-1 rounded-xl border border-border">
                                <button onClick={() => { setAiMode('image'); setAiAnalysisResult(null); }} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${aiMode === 'image' ? 'bg-muted text-white shadow' : 'text-muted-foreground hover:text-white'}`}>Bild</button>
                                <button onClick={() => { setAiMode('link'); setAiAnalysisResult(null); }} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${aiMode === 'link' ? 'bg-muted text-white shadow' : 'text-muted-foreground hover:text-white'}`}>Link / Text</button>
                            </div>
                            {!aiAnalysisResult ? (
                                <>
                                    {aiMode === 'image' && (
                                        <div className="relative w-full aspect-square rounded-xl border-2 border-dashed border-border bg-muted flex flex-col items-center justify-center overflow-hidden group">
                                            {aiImagePreview ? (<><img src={aiImagePreview} className="w-full h-full object-contain p-2" /><div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => { setAiImagePreview(null); setAiSelectedFile(null); }} className="p-2 bg-red-500/20 text-red-200 rounded-full hover:bg-red-500/40"><Trash2 size={20} /></button></div></>) : (<div className="flex flex-col items-center gap-3 text-muted-foreground"><FileImage size={40} strokeWidth={1.5} /><div className="flex flex-col gap-2 items-center"><button onClick={() => aiFileInputRef.current?.click()} className="px-4 py-2 bg-muted hover:bg-muted rounded-lg text-sm text-white transition-colors">Datei wählen</button><span className="text-xs">oder</span><button onClick={handlePasteAiImage} className="px-4 py-2 bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 rounded-lg text-sm transition-colors flex items-center gap-2"><Clipboard size={14} /> Aus Zwischenablage</button></div></div>)}
                                    ...
                                    {aiMode === 'link' && (
                                        <div className="space-y-3 py-6">
                                            ...
                                            <div className="relative"><input className="w-full bg-muted border border-border rounded-xl p-4 pl-12 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-purple-500/50" placeholder="https://shop.lieferant.de/produkt/..." value={aiUrlInput} onChange={(e) => setAiUrlInput(e.target.value)} /><Globe className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" size={20} /></div>
                                        </div>
                                    )}
                                ...
                                <div className="space-y-4 animate-in fade-in">
                                    <div className="p-3 bg-primary/10 border border-emerald-500/20 rounded-xl flex items-center gap-3"><CheckCircle2 size={24} className="text-emerald-400" /><div className="text-sm text-white">Analyse erfolgreich! Wähle einen Namen:</div></div>
                                    <div className="space-y-2">
                                        <div onClick={() => applyAiResult(aiAnalysisResult.name)} className="p-3 rounded-xl border border-border bg-muted hover:bg-muted cursor-pointer transition-colors"><div className="text-[10px] text-muted-foreground uppercase font-bold mb-1">Original (gefunden)</div><div className="font-bold text-white">{aiAnalysisResult.name}</div></div>
                                    ...
                                    <button onClick={() => setAiAnalysisResult(null)} className="w-full py-2 text-xs text-muted-foreground hover:text-white mt-2">Zurück</button>
                                </div>
                            )}
                        </div>
                    </GlassCard>
            ...
            {isImageReuseModalOpen && (
                <div className="fixed inset-0 z-[190] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-in fade-in zoom-in-95">
                    <GlassCard className="w-full max-w-3xl flex flex-col h-[80vh] p-0 overflow-hidden">
                        <div className="p-5 border-b border-border bg-muted flex justify-between items-center">
                            <div><h3 className="text-lg font-bold text-white">Bild wiederverwenden</h3></div><button onClick={() => setIsImageReuseModalOpen(false)} className="text-muted-foreground hover:text-white"><X size={20} /></button>
                        </div>
            ...
            {duplicateConfirmation && duplicateConfirmation.isOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-in fade-in zoom-in-95">
