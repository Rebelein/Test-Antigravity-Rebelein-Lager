...
const Commissions: React.FC = () => {
    ...
    const [collapsedCategories, setCollapsedCategories] = useState<{ [key: string]: boolean }>(() => {
        // Default to collapsed (true) unless pinned (stored as 'true')
        const isPinned = localStorage.getItem('commission_ready_pinned') === 'true';
        return {
            ready: !isPinned,
            preparing: false, draft: false, returnReady: false, returnPending: false
        };
    });
    ...
    const handleScan = async (code: string) => {
        if (!code) return;
        // Parse code (Format: "COMM:<UUID>" or just "<UUID>")
        const commId = code.startsWith('COMM:') ? code.replace('COMM:', '') : code;

        // Find commission
        let comm = commissions.find(c => c.id === commId);

        if (comm) {
            toast.success(`Kommission gefunden: ${comm.name}`);
            handleOpenDetail(comm);
            setShowScanner(false);
            return;
        }

        // If not in current list (maybe filtered?), fetch it
        toast.info("Suche Kommission in Datenbank...");
        try {
            const { data, error } = await supabase.from('commissions').select('*, suppliers(name)').eq('id', commId).single();
            if (data) {
                const newComm = data as ExtendedCommission;
                // Update local list if possible or just open it
                // We don't easily update 'commissions' state here if it's derived/filtered, 
                // but we can set activeCommission directly?
                // Ideally we should add it to commissions list to avoid UI glitches if we close detail
                // But for now, just opening detail is enough
                handleOpenDetail(newComm);
                setShowScanner(false);
                toast.success("Kommission geladen");
            } else {
                toast.error("Kommission nicht gefunden.");
            }
        } catch (e) {
    ...
    // Memoized active commissions for PrintingSection
    const activeCommissionsForPrint = useMemo(() => {
        return commissions.filter(c => ['Preparing', 'Ready', 'ReturnPending'].includes(c.status));
    }, [commissions]);
    ...
    return (
        ...
        >
            ...
            {/* In-Modal Scanner */}
            <GlassModal isOpen={showScanner} onClose={() => setShowScanner(false)} className="max-w-lg h-[80vh] p-0 overflow-hidden">
                <div className="relative h-full flex flex-col">
                    <div className="absolute top-4 right-4 z-50">
                        <Button variant="secondary" onClick={() => setShowScanner(false)} icon={<X size={20} />} />
                    </div>
