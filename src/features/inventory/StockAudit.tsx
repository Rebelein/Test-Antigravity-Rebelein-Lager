...
const StockAudit: React.FC = () => {
    ...
    const getWarehouseIcon = (type: string) => {
        switch (type) {
            ...
            default: return <WarehouseIcon size={24} className="text-muted-foreground" />;
        }
    };
    ...
    return (
        <div className="h-full overflow-y-auto custom-scrollbar space-y-6 pb-24">
            <header className="mb-6">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-muted-foreground">Lagerbestand prüfen und korrigieren.</p>
                    </div>

                    {/* WAREHOUSE SELECTOR BUTTON */}
                    <button
                        onClick={() => setIsWarehouseModalOpen(true)}
                        className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-muted hover:bg-muted border border-border text-sm transition-all"
                    >
                        <span className="text-muted-foreground">Lager:</span>
                        <span className="font-bold text-foreground max-w-[100px] truncate">{activeWarehouse ? activeWarehouse.name : 'Wählen...'}</span>
            ...
            {/* Tabs */}
            <div className="flex gap-2 p-1 dark:bg-black/20 bg-muted/60 rounded-xl border dark:border-white/5 border-border">
                <button onClick={() => setActiveTab('movement')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 ${activeTab === 'movement' ? 'bg-muted text-foreground shadow' : 'text-muted-foreground'}`}>
                    <Activity size={16} /> Hohe Bewegung
                </button>
                <button onClick={() => setActiveTab('stale')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 ${activeTab === 'stale' ? 'bg-muted text-foreground shadow' : 'text-muted-foreground'}`}>
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {recommendations && (
                        <>
                            <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider px-2">{recommendations.title}</h3>
                            {recommendations.items.length === 0 ? (
                                <div className="text-center py-8 bg-muted rounded-xl border dark:border-white/5 border-border text-muted-foreground">
                                    ...
                                    <GlassCard key={idx} className="flex items-center justify-between group">
                                        {recommendations.type === 'shelf' ? (
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-lg bg-purple-500/20 flex items-center justify-center dark:text-purple-300 text-purple-800 font-bold">
                                                    {idx + 1}
                                                </div>
                                                <div>
                                                    <div className="text-lg font-bold text-foreground">{item.category} / {item.locationName}</div>
                                                    <div className="text-xs text-muted-foreground">Bewegungs-Score: {item.score}</div>
                                                </div>
                                            </div>
                                        ) : (
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-muted-foreground flex gap-2">
                                                        ...
                                                        <span className="dark:text-purple-300 text-purple-800 border-l border-border pl-2 ml-1">Zuletzt: {item.lastCountedAt ? new Date(item.lastCountedAt).toLocaleDateString() : 'Nie'}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
            ...
            {isScannerOpen && (
                <div className="fixed inset-0 z-[170] bg-black flex flex-col">
                    {/* Scanner Header */}
                    <div className="p-4 flex justify-between items-center dark:bg-black/30 bg-muted/70 backdrop-blur-sm z-10 absolute top-0 left-0 right-0">
                        <div className="flex items-center gap-2 text-purple-400 font-bold">
                            <ScanLine /> Inventur: {activeWarehouse?.name}
                        </div>
                        <button onClick={handleCloseScanner} className="p-2 bg-muted rounded-full text-foreground"><X size={24} /></button>
                    </div>
                    ...
                    <div className="flex-1 relative bg-black">
                        ...
                        {!scannedLocation && !showCategorySelector && !selectedArticle && cameraError && (
                            <div className="absolute inset-0 flex items-center justify-center p-6 text-center text-red-400 dark:bg-black/30 bg-muted/70 z-30">
                        ...
                        {showCategorySelector && (
                            <div className="absolute inset-0 bg-background/95 backdrop-blur-sm flex flex-col justify-center p-6 animate-in fade-in zoom-in-95 z-50">
                                <div className="text-center mb-6">
                                    <Split size={48} className="mx-auto dark:text-amber-400 text-amber-800 mb-4" />
                                    <h2 className="text-2xl font-bold text-foreground mb-2">Mehrdeutiges Fach!</h2>
                                ...
                                <div className="space-y-3 max-w-sm mx-auto w-full">
                                    {ambiguousCategories.map(cat => (
                                        <button
                                            ...
                                            className="w-full p-4 rounded-xl bg-muted hover:bg-muted border border-border flex justify-between items-center group transition-all"
                                        >
                                            <span className="font-bold text-foreground text-lg">{cat}</span>
                                            <ArrowRight className="text-muted-foreground group-hover:dark:text-amber-400 text-amber-800 transition-colors" />
                                        </button>
                                    ))}
                                    <button onClick={handleCloseScanner} className="w-full py-4 text-muted-foreground hover:text-foreground mt-4">Abbrechen</button>
                                </div>
                            </div>
                        )}
                        ...
                        {scannedLocation && !selectedArticle && (
                            <div className="absolute inset-0 bg-background flex flex-col pt-20 p-4">
                                <h2 className="text-xl font-bold text-foreground mb-4 flex items-center gap-2">
                                    <Layers size={20} className="text-purple-400" /> {scannedLocation}
                                </h2>
                                <p className="text-muted-foreground text-sm mb-4">Bitte Artikel wählen zur Zählung:</p>
                                <div className="space-y-3 overflow-y-auto pb-20">
                                    {locationArticles.map(article => (
                                        <GlassCard key={article.id} onClick={() => handleArticleSelect(article)} className="flex items-center gap-4 cursor-pointer hover:bg-muted">
                                            <div className="w-12 h-12 rounded dark:bg-black/30 bg-muted/70 overflow-hidden">
                                                <img src={article.image} className="w-full h-full object-cover" />
                                            </div>
                                            <div className="flex-1">
                                                <div className="font-bold text-foreground">{article.name}</div>
                                                <div className="text-xs text-muted-foreground">{article.sku}</div>
                                            </div>
                                            <div className="dark:text-emerald-400 text-emerald-800 font-mono">{article.stock}</div>
                                        </GlassCard>
                                    ))}
                                </div>
                            </div>
                        ...
                        {/* RESULT: COUNT ENTRY */}
                        {selectedArticle && (
                            <div className="absolute inset-0 bg-background flex flex-col justify-center p-6 animate-in slide-in-from-bottom-10">
                                <div className="w-full max-w-md mx-auto bg-muted border border-border rounded-3xl p-6 backdrop-blur-sm shadow-2xl">
                                    <div className="text-center mb-6">
                                        <div className="w-20 h-20 rounded-2xl dark:bg-black/30 bg-muted/70 mx-auto mb-4 overflow-hidden border border-border">
                                            <img src={selectedArticle.image} className="w-full h-full object-cover" />
                                        </div>
                                        <h2 className="text-xl font-bold text-foreground line-clamp-2">{selectedArticle.name}</h2>
                                        <div className="flex items-center justify-center gap-2 mt-3 p-2 bg-muted rounded-xl border dark:border-white/5 border-border">
                                    ...
                                    <div className="space-y-6">
                                        <div className="dark:bg-black/30 bg-muted/70 rounded-2xl p-4 flex items-center justify-between">
                                            <button onClick={() => setAuditCount(c => Math.max(0, c - 1))} className="w-12 h-12 rounded-xl bg-muted hover:bg-muted flex items-center justify-center text-foreground text-2xl">-</button>
                                            <div className="text-center">
                                                <div className="text-4xl font-bold text-foreground">{auditCount}</div>
                                                <div className="text-[10px] text-muted-foreground uppercase tracking-wider mt-1">Ist-Menge</div>
                                            </div>
                                            <button onClick={() => setAuditCount(c => c + 1)} className="w-12 h-12 rounded-xl bg-muted hover:bg-muted flex items-center justify-center text-foreground text-2xl">+</button>
                                        </div>
            ...
            {isWarehouseModalOpen && (
                <div className="fixed inset-0 z-[180] flex items-center justify-center p-4 dark:bg-black/30 bg-muted/70 backdrop-blur-sm animate-in fade-in">
                    <GlassCard className="w-full max-w-2xl">
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                <h2 className="text-xl font-bold text-foreground">Lager auswählen</h2>
                                <p className="text-muted-foreground text-sm">Welches Lager wird inventarisiert?</p>
                            </div>
                            {activeWarehouseId && (
                                <button onClick={() => setIsWarehouseModalOpen(false)} className="p-2 bg-muted rounded-full text-muted-foreground hover:text-foreground"><X size={20} /></button>
                            )}
                        </div>
                            ...
                            {warehouses.map(w => {
                                ...
                                return (
                                    <button
                                     ...
                                     ${isActive ? 'ring-2 ring-white scale-[1.02]' : 'hover:scale-[1.02] hover:bg-muted'}
                                   ...
                                   `}
                                    >
                                        ...
                                        <span className="font-bold text-foreground text-center line-clamp-2">{w.name}</span>
                                        <span className="text-[10px] text-muted-foreground uppercase mt-1">{w.type === 'Main' ? 'Hauptlager' : w.type === 'Vehicle' ? 'Fahrzeug' : 'Baustelle'}</span>
