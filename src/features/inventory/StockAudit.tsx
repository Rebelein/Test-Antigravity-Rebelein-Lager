    ...
    return (
        <div className="h-full overflow-y-auto custom-scrollbar space-y-6 pb-24">
            ...
            <div className="flex gap-2 p-1 bg-black/20 rounded-xl border border-white/5">
                <button onClick={() => setActiveTab('movement')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 ${activeTab === 'movement' ? 'bg-muted text-white shadow' : 'text-muted-foreground'}`}>
                    <Activity size={16} /> Hohe Bewegung
                </button>
                <button onClick={() => setActiveTab('stale')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-2 ${activeTab === 'stale' ? 'bg-muted text-white shadow' : 'text-muted-foreground'}`}>
                    ...
                    {recommendations && (
                        <>
                            ...
                            {recommendations.items.length === 0 ? (
                                <div className="text-center py-8 bg-muted rounded-xl border border-white/5 text-muted-foreground">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-muted-foreground flex gap-2">
                                                        ...
                                                        <span className="text-purple-300 border-l border-border pl-2 ml-1">Zuletzt: {item.lastCountedAt ? new Date(item.lastCountedAt).toLocaleDateString() : 'Nie'}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
            ...
            {isScannerOpen && (
                <div className="fixed inset-0 z-[170] bg-black flex flex-col">
                    {/* Scanner Header */}
                    <div className="p-4 flex justify-between items-center bg-black/30 backdrop-blur-sm z-10 absolute top-0 left-0 right-0">
                        <div className="flex items-center gap-2 text-purple-400 font-bold">
                            <ScanLine /> Inventur: {activeWarehouse?.name}
                        </div>
                        <button onClick={handleCloseScanner} className="p-2 bg-muted rounded-full text-white"><X size={24} /></button>
                    </div>
                    ...
                    <div className="flex-1 relative bg-black">
                        ...
                        {!scannedLocation && !showCategorySelector && !selectedArticle && cameraError && (
                            <div className="absolute inset-0 flex items-center justify-center p-6 text-center text-red-400 bg-black/30 z-30">
                                ...
                                <div className="space-y-3 max-w-sm mx-auto w-full">
                                    {ambiguousCategories.map(cat => (
                                        ...
                                        >
                                            ...
                                            <ArrowRight className="text-muted-foreground group-hover:text-amber-400 transition-colors" />
                                        </button>
                                    ))}
                                    <button onClick={handleCloseScanner} className="w-full py-4 text-muted-foreground hover:text-white mt-4">Abbrechen</button>
                                </div>
                            </div>
                        )}
                        ...
                        {selectedArticle && (
                            <div className="absolute inset-0 bg-background flex flex-col justify-center p-6 animate-in slide-in-from-bottom-10">
                                <div className="w-full max-w-md mx-auto bg-muted border border-border rounded-3xl p-6 backdrop-blur-sm shadow-2xl">
                                    <div className="text-center mb-6">
                                        <div className="w-20 h-20 rounded-2xl bg-black/30 mx-auto mb-4 overflow-hidden border border-border">
                                        ...
                                        <div className="flex items-center justify-center gap-2 mt-3 p-2 bg-muted rounded-xl border border-white/5">
                                    ...
                                    <div className="space-y-6">
                                        <div className="bg-black/30 rounded-2xl p-4 flex items-center justify-between">
                                            <button onClick={() => setAuditCount(c => Math.max(0, c - 1))} className="w-12 h-12 rounded-xl bg-muted hover:bg-muted flex items-center justify-center text-white text-2xl">-</button>
                                            ...
                                            <button onClick={() => setAuditCount(c => c + 1)} className="w-12 h-12 rounded-xl bg-muted hover:bg-muted flex items-center justify-center text-white text-2xl">+</button>
                                        </div>
            ...
            {isWarehouseModalOpen && (
                <div className="fixed inset-0 z-[180] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-in fade-in">
                    <GlassCard className="w-full max-w-2xl">
                        <div className="flex justify-between items-start mb-6">
                            ...
                            {activeWarehouseId && (
                                <button onClick={() => setIsWarehouseModalOpen(false)} className="p-2 bg-muted rounded-full text-muted-foreground hover:text-white"><X size={20} /></button>
                            )}
                        </div>
