...
import { LabelConfig, GroupedLocation, DownloadFormat, LabelViewMode, LocationOccupancyFilter, DEFAULT_LABEL_CONFIG } from '../../features/labels/types';
import { drawLabelToCanvas } from '../../features/labels/utils/LabelCanvasHelper';
import { handlePrintPDFHelper } from '../../features/labels/utils/LabelPDFHelper';
...
const Labels: React.FC = () => {
    ...
    const handlePrintPDF = (singleMode: boolean = false) => {
        const itemsToPrint = singleMode
            ...
            : selectedItemsList;
        handlePrintPDFHelper(itemsToPrint, getConfigForItem, singleMode, previewItem);
    };
        ...
        <div className="space-y-6 pb-20 h-full overflow-y-auto lg:overflow-visible">
            ...
            {loading ? <div className="flex justify-center py-12"><Loader2 className="animate-spin text-blue-400" /></div> : (
                <div className="flex flex-col lg:flex-row gap-6 h-auto lg:h-[calc(100vh-200px)] min-h-0 lg:min-h-[650px] shrink-0">
                    ...
                    {/* LEFT: SELECTION */}
                    <GlassCard
                        className="flex-none lg:flex-1 h-[500px] lg:h-auto overflow-hidden border border-border"
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-1">
                            {labelViewMode === 'articles' ? (
                                filteredLabelArticles.length === 0 ? (
                                    <div className="text-center text-muted-foreground py-8 text-sm">Keine Artikel gefunden.</div>
                                ) : (
                                    filteredLabelArticles.map(article => (
                                        <div
                                            key={article.id}
                                            onClick={() => toggleSelection(article.id)}
                                            className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${selectedArticleIds.has(article.id) ? 'bg-primary/10 border border-emerald-500/20' : 'hover:bg-muted border border-transparent'}`}
                                        >
                                            <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${selectedArticleIds.has(article.id) ? 'bg-primary border-emerald-500 text-white' : 'border-border'}`}>
                                ...
                                filteredLocations.length === 0 ? (
                                    <div className="text-center text-muted-foreground py-8 text-sm">Keine Lagerfächer gefunden.</div>
                                ) : (
                                    filteredLocations.map(loc => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-primary/10 border border-emerald-500/20' : 'hover:bg-muted border border-transparent'}`}
                                            >
                                                <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isSelected ? 'bg-primary border-emerald-500 text-white' : 'border-border'}`}>
                    ...
                    <div className="flex-none lg:flex-1 flex flex-col gap-4 min-w-[min(100%,320px)]">
                        ...
                        <GlassCard className="p-4 space-y-4 flex-1 flex flex-col">
                                    ...
                                    (() => {
                                        ...
                                        return (
                                            <>
                                                ...
                                                <div className="text-muted-foreground text-xs mt-4 text-center w-full">Vorschau ({config.width}mm x {config.height}mm)</div>
                            ...
                            {selectedItemsList.length > 0 && (
                                <>
                                    {/* Navigation */}
                                    <div className="flex items-center justify-between bg-muted rounded-lg p-2 border border-border">
                                        <button onClick={() => setPreviewIndex(prev => Math.max(0, prev - 1))} disabled={previewIndex === 0} className="p-1 hover:bg-muted rounded disabled:opacity-30"><ChevronLeft size={20} /></button>
                                        <span className="text-xs text-muted-foreground font-mono">{previewIndex + 1} / {selectedItemsList.length}</span>
                                        <button onClick={() => setPreviewIndex(prev => Math.min(selectedItemsList.length - 1, prev + 1))} disabled={previewIndex === selectedItemsList.length - 1} className="p-1 hover:bg-muted rounded disabled:opacity-30"><ChevronRight size={20} /></button>
                                    </div>

                                    {/* Manual Sliders */}
                                    {previewItem && (isCustomSize || labelOverrides['id' in previewItem ? previewItem.id : previewItem.uniqueKey]) && (
                                        <div className="space-y-3 pt-2 animate-in slide-in-from-top-2 fade-in bg-muted p-3 rounded-xl">
                                            <div className="flex items-center justify-between">
                                                <label className="text-xs text-muted-foreground flex items-center gap-1"><Edit2 size={10} /> Individuell anpassen</label>
                                                <button onClick={toggleOverrideForCurrent} className={`w-8 h-4 rounded-full transition-colors relative ${labelOverrides['id' in previewItem ? previewItem.id : previewItem.uniqueKey] ? 'bg-primary' : 'bg-muted'}`}>
                                            ...
                                            <div className="grid grid-cols-3 gap-3">
                                                <div>
                                                    <label className="text-[10px] text-muted-foreground block mb-1">Breite</label>
                                                    <input type="range" min="30" max="100" step="1" className="w-full h-1 bg-muted rounded-lg appearance-none cursor-pointer" value={getConfigForItem('id' in previewItem ? previewItem.id : previewItem.uniqueKey).width} onChange={(e) => updateCurrentConfig({ width: Number(e.target.value) })} />
                                                </div>
                                                <div>
                                                    <label className="text-[10px] text-muted-foreground block mb-1">Höhe</label>
                                                    <input type="range" min="20" max="80" step="1" className="w-full h-1 bg-muted rounded-lg appearance-none cursor-pointer" value={getConfigForItem('id' in previewItem ? previewItem.id : previewItem.uniqueKey).height} onChange={(e) => updateCurrentConfig({ height: Number(e.target.value) })} />
                                                </div>
                                                <div>
                                                    <label className="text-[10px] text-muted-foreground block mb-1">Schrift</label>
                                                    <input type="range" min="0.5" max="1.5" step="0.1" className="w-full h-1 bg-muted rounded-lg appearance-none cursor-pointer" value={getConfigForItem('id' in previewItem ? previewItem.id : previewItem.uniqueKey).fontSizeScale} onChange={(e) => updateCurrentConfig({ fontSizeScale: Number(e.target.value) })} />
                                                </div>
                                            </div>
                                        </div>
                                    )}
                        ...
                        <div className="bg-black/20 border border-border rounded-xl p-3">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <Button
                                    variant="secondary"
                                    className="col-span-2 bg-muted hover:bg-muted"
                                ...
                                {/* Download All */}
                                <Button
                                    className={downloadFormat === 'png' ? 'bg-blue-600 hover:bg-blue-500' : 'bg-primary hover:bg-primary'}
