        ...
        {suppliers.map((supplier) => (
          <GlassCard key={supplier.id} className="relative group">
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-purple-500/10 border border-purple-500/20 dark:text-purple-300 text-purple-800"><Factory size={24} /></div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-muted-foreground mt-1">
                  ...
                  {supplier.csv_format && <div className="flex items-center gap-1 dark:text-emerald-400 text-emerald-800"><FileText size={12} /> CSV Vorlage aktiv</div>}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => handleOpenEdit(supplier)} className="p-2 rounded-lg bg-muted hover:bg-muted text-muted-foreground hover:text-foreground"><Edit2 size={16} /></button>
                <button onClick={() => handleDelete(supplier.id, supplier.name)} className="p-2 rounded-lg bg-muted hover:bg-red-500/20 text-muted-foreground hover:dark:text-red-300 text-red-800"><Trash2 size={16} /></button>
              </div>
            </div>
          </GlassCard>
        ))}
        {suppliers.length === 0 && <div className="text-center text-muted-foreground py-8">Keine Lieferanten angelegt.</div>}
      </div>
        ...
        <div className="fixed inset-0 z-[170] flex items-center justify-center p-4 dark:bg-black/30 bg-muted/70 backdrop-blur-sm">
          <GlassCard className="w-full max-w-lg" title={editingId ? "Lieferant bearbeiten" : "Neuer Lieferant"}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 mt-2">
              <div><label className="block text-sm text-muted-foreground mb-1">Firmenname</label><GlassInput value={supplierForm.name} onChange={e => setSupplierForm({ ...supplierForm, name: e.target.value })} required /></div>
              <div><label className="block text-sm text-muted-foreground mb-1">Kundennummer</label><GlassInput value={supplierForm.customer_number} onChange={e => setSupplierForm({ ...supplierForm, customer_number: e.target.value })} /></div>
              <div className="pt-4 border-t border-border">
                <label className="block text-sm font-medium dark:text-emerald-400 text-emerald-800 mb-1 flex items-center gap-2"><FileText size={14} /> CSV Export Vorlage</label>
                <p className="text-[10px] text-muted-foreground mb-2">Definiere das Zeilenformat für den Export. Platzhalter: <code>&#123;&#123;sku&#125;&#125;</code>, <code>&#123;&#123;amount&#125;&#125;</code>, <code>&#123;&#123;name&#125;&#125;</code>.</p>
                <textarea
                  className="w-full h-24 bg-muted border border-border rounded-xl p-3 text-xs font-mono text-foreground dark:placeholder-white/20 placeholder-muted-foreground/80 focus:outline-none focus:ring-1 focus:ring-emerald-500"
