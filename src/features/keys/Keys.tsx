...
const KeyCard: React.FC<{
    ...
    >
        ...
        <h3 className="font-bold text-foreground text-sm mb-1 truncate pl-2" title={keyData.name}>{keyData.name}</h3>
        ...
        {keyData.status === 'InUse' ? (
            <div className="text-xs dark:text-amber-200 text-amber-900/60 pl-2 truncate flex items-center gap-1 mt-auto bg-amber-500/10 -mx-3 -mb-3 p-2 pt-2.5 mt-2">
                <User size={12} className="shrink-0" /> <span className="font-medium">{keyData.holder_name || 'Unbekannt'}</span>
            </div>
        ) : (
            <div className="text-xs dark:text-emerald-400 text-emerald-800/60 pl-2 truncate mt-auto bg-primary/5 -mx-3 -mb-3 p-2 pt-2.5 mt-2 flex items-center gap-1">
            ...
            {categories.map(cat => {
                ...
                <button
                    ...
                    className={clsx(
                        ...
                        selectedCategory === cat.id ? "bg-primary/20 dark:text-emerald-400 text-emerald-800 border border-emerald-500/30" : "text-muted-foreground hover:text-foreground hover:bg-muted"
                    )}
                >
                    <ChevronRight size={14} className={clsx("transition-transform shrink-0 mt-1", selectedCategory === cat.id ? "rotate-90 dark:text-emerald-400 text-emerald-800" : "text-muted-foreground group-hover:text-muted-foreground")} />
            ...
            {(() => {
                ...
                <button
                    ...
                    className={clsx(
                        ...
                        selectedCategory === 'uncategorized' ? "bg-primary/20 dark:text-emerald-400 text-emerald-800 border border-emerald-500/30" : "text-muted-foreground hover:text-foreground hover:bg-muted"
                    )}
                >
                    <ChevronRight size={14} className={clsx("transition-transform shrink-0 mt-1", selectedCategory === 'uncategorized' ? "rotate-90 dark:text-emerald-400 text-emerald-800" : "text-muted-foreground group-hover:text-muted-foreground")} />
        ...
        <div className="relative h-full flex flex-col overflow-hidden">
            ...
            <div className="flex h-full overflow-hidden mt-2">
                ...
                <div className="flex-1 h-full overflow-hidden flex flex-col lg:pl-4">
                    {/* Stats / Tabs */}
                    <div className="flex gap-2 p-1 dark:bg-black/20 bg-muted/60 rounded-xl border dark:border-white/5 border-border mb-4 shrink-0">
                        <button onClick={() => { setActiveTab('all'); setSelectedCategory(null); }} className={`flex-1 py-2 rounded-lg text-sm font-medium ${activeTab === 'all' ? 'bg-muted text-foreground' : 'text-muted-foreground'}`}>Gesamt <span className="text-[10px] bg-muted px-1 rounded">{keys.length}</span></button>
                        <button onClick={() => { setActiveTab('available'); setSelectedCategory(null); }} className={`flex-1 py-2 rounded-lg text-sm font-medium ${activeTab === 'available' ? 'bg-primary/20 dark:text-emerald-300 text-emerald-800' : 'text-muted-foreground'}`}>Verfügbar <span className="text-[10px] bg-muted px-1 rounded">{keys.filter(k => k.status === 'Available').length}</span></button>
                        <button onClick={() => { setActiveTab('inUse'); setSelectedCategory(null); }} className={`flex-1 py-2 rounded-lg text-sm font-medium ${activeTab === 'inUse' ? 'bg-amber-500/20 dark:text-amber-300 text-amber-800' : 'text-muted-foreground'}`}>Ausgegeben <span className="text-[10px] bg-muted px-1 rounded">{keys.filter(k => k.status === 'InUse').length}</span></button>
                    </div>

                    {/* Search */}
                    <div className="relative mb-4 shrink-0">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={18} />
                        <input className="w-full bg-muted border border-border rounded-xl py-3 pl-10 pr-4 text-foreground focus:outline-none focus:border-emerald-500 transition-colors" placeholder="Suche..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                    </div>
