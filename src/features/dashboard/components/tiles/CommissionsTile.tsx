        ...
        <>
            ...
            <div className="flex md:hidden border-b border-white/10 shrink-0">
                {(['open', 'backlog', 'returns'] as const).map((tab) => {
                    ...
                    const barColor = tab === 'open' ? 'bg-emerald-500' : tab === 'backlog' ? 'bg-rose-500' : 'bg-purple-500';
                    return (
                        <button
                            key={tab}
                            onClick={() => onMobileTabChange(tab)}
                            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wide transition-colors relative ${mobileTab === tab ? activeColor : 'text-white/40'}`}
                        >
                            {label}
